...
export default function ModuleGrid({
  ...
  return (
    <section className="space-y-4">
      <div>
        <div className="text-xs font-mono uppercase tracking-[0.18em] text-[var(--text-3)]">Available modules</div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => {
          ...
          const classes = isApiRoute
            ? "rounded-[24px] border border-[var(--border)] bg-[var(--card)] p-5"
            : "group rounded-[24px] border border-[var(--border)] bg-[var(--card)] p-5 transition-transform hover:-translate-y-0.5"
          ...
          const content = (
            <>
              <div className="flex items-start justify-between gap-3">
                ...
                {isApiRoute ? (
                  <div className="rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-3)]">
                    API only
                  </div>
                ) : (
              ...
              {isApiRoute ? <p className="mt-2 text-xs leading-5 text-[var(--text-3)]">โมดูลนี้ใช้งานผ่านแบบฟอร์มหรือ workbench ในหน้านี้ ไม่พาผู้ใช้เปิด endpoint ตรงแล้ว</p> : null}
              {item.stats?.length ? (
          ...
          return isApiRoute ? (
            <article key={item.href + item.label} className={classes}>
              {content}
            </article>
          ) : (
