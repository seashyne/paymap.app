...
function normalizePriceIds(priceIds: { monthly?: string; yearly?: string } | null) {
  if (!priceIds) return null
  const monthly = priceIds.monthly?.trim() ?? ""
  const yearly = priceIds.yearly?.trim() ?? ""
...
export async function GET() {
  ...
  return ok({
    ...
    personalPlans: (["free", "pro", "family"] as const).map((k) => ({
      ...
      priceIds: k === "free" ? null : normalizePriceIds(prices.personal[k]),
    })),
    businessPlans: (["free", "sme", "scale", "enterprise"] as const).map((k) => ({
      key: k,
      ...BUSINESS_PRICING[k],
      limits: BUSINESS_PLAN_LIMITS[k],
      priceIds: k === "free" || k === "enterprise" ? null : normalizePriceIds(prices.business[k]),
    })),
    merchantPlans: (["free", "starter", "growth", "scale", "enterprise"] as const).map((k) => ({
      key: k,
      ...MERCHANT_PRICING[k],
      limits: MERCHANT_PLAN_LIMITS[k],
      priceIds: k === "free" || k === "enterprise" ? null : normalizePriceIds(prices.merchant[k]),
    })),
  })
}
